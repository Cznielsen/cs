...
static void timer_funct(unsigned long funct_parameter) {
  ...
  /*struct my_dev *d = dev_get_drvdata(funct_parameter);
  int state = d->state;
  int rate = d->rate;*/
...
static int my_devt_probe(struct platform_device *pdev){
  ...
  struct my_dev drvdata = { .state = 0, .rate = 1 };
  init_timer(&timer);
  ...
  struct device *my_device = device_create(test_class, NULL, MKDEV(MAJOR(devno), 0), &drvdata, "mytestdriver%d", 0);
