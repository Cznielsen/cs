  ...
  (lambda (x y)
    (cond [(null? x) '()]                                ;;Basis: Hvis et sæt er tomt, så returneres det tomme sæt.
          [(not (member (car x) y))                      ;;Induktion: Hvis car(x) ikke er en del af sættet y:
           (cons (car x)(set-difference (cdr x) y))]     ;;Så er forskellen mellem to sæt, x og y, = sættet som består af car(x) og forskellen mellem car(x) og y.
          [else                                          ;;Hvis car(x) ér en del af y, så:
           (set-difference (cdr x) y)])))                ;;er forskellen mellem x og y det samme som forskellen mellem car(x) og y.
