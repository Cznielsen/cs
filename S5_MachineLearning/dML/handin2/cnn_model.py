...
class Config(object):
    ...
    ### YOUR CODE HERE
    conv_output_size = int(14*14*32/2)
    ### END CODE
    hidden_size = 1024
    dropout = 0.5 #
    weight_decay = 1e-4
...
class ConvolutionalModel(TfModel):
    ...
    def add_placeholders(self):
        ...
        self.weight_decay_placeholder = tf.placeholder(dtype=tf.float32)
        ### END CODE
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.Variable(tf.zeros((28, 28, 32), tf.float32))
        b2 = tf.Variable(tf.zeros((14, 14, 64), tf.float32))
        b3 = tf.Variable(tf.zeros(self.config.hidden_size, tf.float32))
        b4 = tf.Variable(tf.zeros(self.config.n_classes, tf.float32))

        ksize = [1, self.config.pool_sizes[0], self.config.pool_sizes[1], 1]
        l1 = tf.nn.max_pool(tf.nn.relu(tf.nn.conv2d(input=x_image, filter=C1, strides=[1, 1, 1, 1], padding='SAME') + b1),
                            ksize=ksize,
                            strides=ksize,
                            padding='SAME')
        l2 = tf.nn.max_pool(tf.nn.relu(tf.nn.conv2d(input=l1, filter=C2, strides=[1, 1, 1, 1], padding='SAME') + b2),
                            ksize=ksize,
                            strides=ksize,
                            padding='SAME')

        f = tf.contrib.layers.flatten(l2)
