...
class LogisticClassifierTwoClass(MlModel):
    ...
    def predict(self, X_):
        ...
        ### YOUR CODE HERE
        z = X.dot(self.w)
        predictions = log_reg.logistic(z)
...
class LogisticClassifier(MlModel):
    ...
    def train(self, X, y, reg=1e-4, lr=0.1, epochs=5, batch_size=16):
        ...
        for classs in range(len(self.classes)):
            model = LogisticClassifierTwoClass()
            z = np.array([1 if label == classs else 0 for label in y])
            model.train(X, z, reg=reg, lr=lr, epochs=epochs, batch_size=batch_size)
...
def model_accuracy(model, X, y):
    """ Compute the accuracy of model defined by w on data X, Y
...
def run_validation(model, X, y, params, val_size=0.2, **kwargs):
    ...
    acc = np.zeros(len(params))
    ### YOUR CODE HERE 5-10 lines
    models = [copy.copy(model) for i in range(len(params))]
    [model.train(train, target, reg=params[idx]) for idx, model in enumerate(models)]
    acc = np.array([model_accuracy(model, val_train, val_target) for model in models])
    model = models[np.argmax(acc)]
    ### END CODE
    return model, acc
