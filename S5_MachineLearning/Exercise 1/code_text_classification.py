...
def standardize_strings(string_list):
    ...
    ### YOUR CODE 1-3 lines
    standardize = lambda s: str.lower(s).replace(".", "").replace(",", "").replace("-", "").replace("_", "").replace(
        "(", "").replace(")", "")
    res = [standardize(i) for i in string_list]
...
def split_strings(strings):
    ...
    ### YOUR CODE
    word_lists = [i.split() for i in strings]
    ### END CODE
...
def remove_irrelevant_words(word_lists, bad_words=set()):
    ...
    ### YOUR CODE, 1-3 lines
    pruned_word_lists = [[word for word in list if word not in bad_words] for list in word_lists]
    ### END CODE
...
class TextClassifier():
    ...
    def __init__(self):
        ...
        self.stopwords = {'over', 'hvad', 'man', 'sit', 'også', 'han', 'eller', 'blive', 'denne', 'det', 'meget', 'til',
                          'anden', 'hende', 'os', 'hvis', 'op', 'din', 'alt', 'havde', 'fra', 'en', 'af', 'efter',
                          'dig', 'mig', 'ind', 'hvor',
                          'den', 'alle', 'ikke', 'var', 'at', 'her', 'du', 'hos', 'jeg', 'der', 'ville', 'thi', 'ham',
                          'vil', 'under',
                          'ad', 'på', 'have', 'disse', 'for', 'skal', 'et', 'bliver', 'min', 'sig', 'i', 'nu', 'og',
                          'har', 'mine', 'vi',
                          'da', 'selv', 'jer', 'mit', 'sådan', 'hans', 'skulle', 'noget', 'som', 'vor', 'hendes', 'om',
                          'kunne', 'nogle',
                          'med', 'være', 'været', 'de', 'sine', 'men', 'dette', 'sin', 'deres', 'jo', 'ud', 'hun', 'er',
                          'ned', 'mange',
    ...
    def make_vocabulary_and_index(self, word_lists):
        ...
        ### YOUR CODE 3-4 lines
        self.vocabulary = {word for sentence in word_lists for word in sentence}
        self.word_to_index = {x: i for x, i in zip(self.vocabulary, range(len(self.vocabulary)))}
        self.index_to_word = {i: x for x, i in zip(self.vocabulary, range(len(self.vocabulary)))}
    ...
    def words_to_vectors(self, word_lists, vocabulary, index_map):
        """
        Each list of strings in word_lists is turned into a vector of length |vocabulary| in such a way
        that the ith entry of the vector is the number of occurences of word i (under index_map) in the string.
        ...
        ### YOUR CODE
        word_temp = [[index_map[word] for word in sentence] for sentence in word_lists]
        word_counter = [Counter(x) for x in word_temp]

        for idx, counter in enumerate(word_counter):
            for key in counter.keys():
                word_vectors[idx, key] = counter[key]
        ### END CODE
    ...
    def compute_class_word_probabilities(self, vectors):
        ...
        for i in range(self.num_classes):
            label_filter = self.labels[:] == i
            sentences = vectors[label_filter, :]
            amount_of_w = [np.sum(x) for x in np.transpose(sentences)]
            words_in_class_c = np.sum(amount_of_w)

            class_probs[i] = [(1 + x) / (words_in_class_c + len(self.vocabulary)) for x in amount_of_w]

            prior[i] = np.sum(label_filter) / len(self.labels)
    ...
    def predict(self, strings):
        ...
        for i, c in enumerate(cleaned_strings):
            ...
            indexing_words = [self.word_to_index[x] for x in c if x in self.vocabulary]
            summed = [np.sum([np.log(prob_words[word]) for word in indexing_words]) + np.log(self.prior(idx)) for idx, prob_words in enumerate(self.class_probabilities)]
            res[i] = np.argmax(summed)
            ### END CODE
...
def test_string_cleaning():
    ...
    expect = ['i think machine learning is cool', 'he likes cake cake cake',
              'pandora has a box that should not be opened']
